...
function ServerInput(props: {
  ...
  async function fetchSessionToken(host: string) {
    const session_token = await toast.promise(
      onLogin(host, form.email, form.password),
      {
  ...
  async function fetchCollections(session_token: string) {
    ...
    const collections = await toast.promise(
      onCollectionsList(host, session_token),
      {
  ...
  async function addServerClick() {
    ...
    if (form.database != "-1") {
      ...
      const schema = await toast.promise(
        onDBSchemaFetch(host, form.session_token, form.database),
        {
      ...
      <div className="flex flex-col">
        ...
        <div
          className={`relative w-full mb-4 ${
            databasesList.length === 0 ? "hidden" : "block"
          }`}
        >
          <label className="block text-gray-700 text-sm font-bold mb-2">
            Database
        ...
        <div
          className={`relative w-full mb-4 ${
            collectionsList.length === 0 ? "hidden" : "block"
          }`}
        >
          <label className="block text-gray-700 text-sm font-bold mb-2">
            Collections
            ...
            <ControlledTreeEnvironment
              items={{
                ...
                root: {
                  index: "root",
                  isFolder: true,
                  children: collectionsList.map((collection) =>
  ...
  return (
    <div
      className={`mt-3 text-xl leading-8 grid grid-cols-1 ${
        servers.length > 0 && "sm:grid-cols-3"
      } gap-6`}
    >
      {servers.map((server) => (
